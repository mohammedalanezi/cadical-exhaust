...
#endif
                        "  --observe <x1> <x2> ...    only do an exhaustive search over x1, x2, etc.\n"
...
int App::main (int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp (argv[i], "-h") || !strcmp (argv[i], "--help") ||
            ...
            if (++i == argc)
                APPERR("argument to '--observe' missing");
            else {
                int temp_val;
                std::cout << "c observing variable: ";
        ...
        else if (!strcmp (argv[i], "--output-solutions")) {
        output_solutions = true;
        std::cout << "c output-solutions = true" << endl;
        }
#ifndef __WIN32
